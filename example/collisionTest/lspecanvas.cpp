...
void LspeCanvas::query(Object *obj) {
    ...
    man->query(
        [](const lspe::abt::node *node, void *extra) -> bool {
            using namespace lspe::shape;

            std::array<const char *, 5> stype{
                "Line", "Circle", "Polygen", "Ellipse"};
            ...
            if (p->index != q->index) {
                ...
                collider.bindSupports(
                    supports[static_cast<int>(p->shape.type)],
                    supports[static_cast<int>(q->shape.type)]);
                collider.bindInitialGenerator(
                    [](lspe::Shape,
                       lspe::Shape,
                       const lspe::vec2 &,
                       void *extra) -> lspe::vec2 {
                ...
                if (collider.collided()) {
                    if (p->index == e->last[0] && q->index == e->last[1]
                        ...
                        qDebug()
                            ...
                            << stype[static_cast<int>(p->shape.type)] << "x"
                            << stype[static_cast<int>(q->shape.type)] << "["
                            << p->index << ":" << q->index << "]";
                        e->last[0] = p->index;
                        e->last[1] = q->index;
                    }
...
lspeman *LspeCanvas::setup() {
    ...
    { //! Line
        const size_t N = 4;
        for (int i = 0; i < N; ++i) { man->newLine(); }
        qDebug() << "Added" << N << "Lines";
    }

    { //! CIRCLE
        const size_t N = 4;
        for (int i = 0; i < N; ++i) { man->newCircle(); }
        qDebug() << "Added" << N << "Circles";
    }

    { //! Polygen
        const size_t N = 16;
        for (int i = 0; i < N; ++i) { man->newPolygen(); }
        qDebug() << "Added" << N << "Polygens";
    }

    { //! Ellipse
        const size_t N = 4;
        for (int i = 0; i < N; ++i) { man->newEllipse(); }
