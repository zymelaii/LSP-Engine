...
void LspeCanvas::paintEvent(QPaintEvent *event) {
    if (initialized) {
        ...
        for (auto obj : solver.getObjects()) { drawObject(obj); }
...
void LspeCanvas::drawObject(lspe::RigidBody *obj) {
    ...
    switch (obj->getShape().type) {
        ...
        case lspe::ShapeType::ePolygen: {
            ...
            points.clear();
            for (auto p : e->vertices) { points.push_back(QPointF(p.x, p.y)); }
        ...
        case lspe::ShapeType::eEllipse: {
            ...
            painter->translate(-center.x, -center.y);
            painter->drawEllipse(center.x - rx, center.y - ry, rx * 2, ry * 2);
...
void LspeCanvas::setup() {
    ...
    LSPE_DEBUG(
        "TriangleBody: AngularVelocity = %f rad/s",
        body->getProperty().angularVelocity);
