...
void freeShape(Shape shape) {
    switch (shape.type) {
        case ShapeType::eLine:
            delete (Line *)(shape.data);
            break;
        case ShapeType::eCircle:
            delete (Circle *)(shape.data);
            break;
        case ShapeType::ePolygen:
            delete (Polygen *)(shape.data);
            break;
        case ShapeType::eEllipse:
            delete (Ellipse *)(shape.data);
            break;
        case ShapeType::eBezier2:
            delete (Bezier2 *)(shape.data);
            break;
        case ShapeType::eBezier3:
            delete (Bezier3 *)(shape.data);
            break;
    };
}
...
void Solver::preSolve() {
    for (auto body : bodys) { body->preUpdate(step); }
}
...
void Solver::inSolve() {
    ...
    for (int i = 0; i < pairsCount; ++i) {
        ...
        uint64_t hash =
            (pairs[i].first & 0xffff) << 16 | (pairs[i].second & 0xffff);

        auto it = std::find_if(
            contacts.begin(), contacts.end(), [hash](const DemoContact &a) {
                return a.hash == hash;
            });

        collider.reset();

        collider.setTestPair(bodys[0]->getShape(), bodys[1]->getShape());
    ...
    for (auto &e : contacts) { //! apply collision response
        ...
        vec2  n   = e.normal;
        float Mn  = 1.0
                 / (ima + cross(ra, n) * cross(ra, n) * iia + imb
                    + cross(rb, n) * cross(rb, n) * iib);

        float lambda = -dot(n, vpa - vpb) * Mn;

        vec2 impulseForce = lambda * n * step;
        LSPE_DEBUG(
            "Apply Collision Impulse: (%f, %f) N*s",
            impulseForce.x,
            impulseForce.y);
...
vec2 Solver::centerOf(Shape shape) {
    ...
    switch (shape.type) {
        case ShapeType::eLine:
            return centroidOf(*(Line *)(shape.data));
        case ShapeType::eCircle:
            return ((Circle *)(shape.data))->center;
        case ShapeType::ePolygen:
            return ((Polygen *)(shape.data))->center;
        case ShapeType::eEllipse:
            return ((Ellipse *)(shape.data))->center;
        default: {
            LSPE_DEBUG(
                "Solver::centerOf: "
...
fnsupport Solver::getDefaultSupport(ShapeType type) {
    ...
    switch (type) {
        ...
        case ShapeType::ePolygen:
            return supportPolygen;
        case ShapeType::eEllipse:
            return supportEllipse;
        default: {
            LSPE_DEBUG("Solver::getDefaultSupport: "
